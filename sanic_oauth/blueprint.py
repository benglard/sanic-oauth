...
async def oauth(request: Request) -> HTTPResponse:
    ...
    token, _data = await client.get_access_token(
        request.args.get("code"), redirect_uri=use_redirect_uri
    )
    ...
    return redirect(
        request.ctx.session.get("after_auth_redirect", use_after_auth_default_redirect)
    )


async def fetch_user_info(
    request, provider, oauth_endpoint_path, local_email_regex
) -> UserInfo:
    try:
...
def login_required(
    ...
    async def wrapped(request, **kwargs):
        ...
        user = await fetch_user_info(
            request, provider, oauth_endpoint_path, email_regex or oauth_email_regex
        )
...
def setup_providers(  # pylint: disable=too-many-locals
    providers_conf: typing.Dict,
    oauth_redirect_uri: str,
    oauth_scope: str,
    oauth_endpoint_path: str,
) -> typing.Dict:
    ...
    for provider_name, provider_conf in providers_conf.items():
        ...
        if p_class_link is None:
            raise OAuthConfigurationException(
                "Provider config must have PROVIDER_CLASS set."
            )
...
def legacy_oauth_configuration(
    ...
    client_setting = {
        ...
        for config_key in sanic_app.config.keys()
        if config_key.startswith("OAUTH")
    }

    if provider_class_link is None:
        raise OAuthConfigurationException(
            "You should setup OAUTH_PROVIDER setting for app"
        )
    if oauth_redirect_uri is None:
        raise OAuthConfigurationException(
            "You should setup OAUTH_REDIRECT_URI setting for app"
        )
    if oauth_scope is None:
        raise OAuthConfigurationException(
            "You should setup OAUTH_SCOPE setting for app"
        )
    if provider_class_link:
        ...
        if module_object is None:
            raise OAuthConfigurationException(
                f"Cannot find module {provider_module_path} to import OAuth provider"
            )
        ...
        if not issubclass(provider_class, Client):
            raise OAuthConfigurationException(
                "Class must be a child of sanic_oauth.core.Client class"
            )
...
def create_oauth_factory(sanic_app: Sanic, _loop) -> None:
    ...
    if providers_conf:
        providers = setup_providers(
            providers_conf, oauth_redirect_uri, oauth_scope, oauth_endpoint_path
        )
        ...
        client_setting, provider_class = legacy_oauth_configuration(
            sanic_app, provider_class_link, oauth_redirect_uri, oauth_scope
        )
